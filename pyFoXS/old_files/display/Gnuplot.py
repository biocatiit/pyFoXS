...
from .utils import trim_extension
...
class Gnuplot:
    ...
    def print_profile_script(pdbs):
        if isinstance(pdbs, list):
            ...
            # file names
            profile_file_name = pdbs + ".dat"
            plt_file_name = trim_extension(pdbs) + ".plt"
            png_file_name = trim_extension(pdbs) + ".png"
            # output script
    ...
    @staticmethod
    def print_canvas_script_pdb(pdbs, max_num):
        # ColorCoder.set_number(len(pdbs))
        hex_color = "#ZZZZZZ"
    ...
    @staticmethod
    def print_canvas_script_fps(fps, max_num):
        ...
        for i in range(min(len(fps), max_num)):
            ...
            pdb_name = trim_extension(fps[i].get_pdb_file_name())
            profile_name = trim_extension(os.path.basename(fps[i].get_profile_file_name()))
    ...
    def print_fit_script(fps):
        ...
        if isinstance(fps, list):
            ...
            pdb_name = trim_extension(fps.get_pdb_file_name())
            profile_name = trim_extension(os.path.basename(fps.get_profile_file_name()))
            ...
            with open(plt_file_name, "w") as plt_file:
                ...
                plt_file.write("plot '{}' u 1:2 t 'Experimental' lc rgb '#333333' pt 6 ps 0.8"
                            ", '{}' u 1:4 t 'FoXS χ^2 = {}' w lines lw 2.5 lc rgb '#e26261'\n".format(fit_file_name, fit_file_name, fps.get_chi_square()))
                ...
                plt_file.write(", '{}' u 1:4 t 'FoXS chi^2 = {}' w lines lw 2.5 lc rgb '#e26261'\n".format(fit_file_name, fps.get_chi_square()))
